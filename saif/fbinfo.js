...
(function () {
  ...
  function injectUI() {
    ...
    // 1. REMOVED `onclick="handleFbInfo()"` FROM THE BUTTON
    box.innerHTML = `
      <div style="
        padding:22px;
      ...
      ">
        ...
        <button id="fbInfoBtn" class="dl-btn">
    ...
    // 2. ADDED EVENT LISTENERS HERE
    const inputField = document.getElementById("fbInfoInput");
    const actionButton = document.getElementById("fbInfoBtn");

    // Handle button click
    actionButton.addEventListener('click', handleFbInfo);
  ...
  // ... rest of your code (extractUID and handleFbInfo functions) remains exactly the same ...
  ...
  window.handleFbInfo = async function () {
      ...
      try {
          ...
          const avatarApi = `https://graph.facebook.com/${encodeURIComponent(uid)}/picture?height=500&width=500&redirect=false&access_token=${accessToken}`;
          const profileUrl = /^\d+$/.test(uid) ? `https://facebook.com/profile.php?id=${uid}` : `https://facebook.com/${uid}`;
          const avatarRes = await fetch(avatarApi);
          const avatarData = await avatarRes.json();
          const avatar = avatarData?.data?.url || `https://graph.facebook.com/${encodeURIComponent(uid)}/picture?height=500&width=500`;
